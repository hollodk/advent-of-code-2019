...
class IntCode
{
    ...
    public function addInput($phase, $input)
    {
        if (is_array($input)) {
            foreach ($input as $o) {
                $this->phases[$phase]->input[] = $o;
            }
    ...
    public function configurePhases($options=null)
    {
        if (!$this->runPhases) {
            $phases = [0];
        } else {
        ...
        foreach ($phases as $loop=>$next) {
            ...
            if ($this->runPhases) {
                ...
                if ($loop == 0 && $this->userInput !== null) {
                    $this->phases[$loop]->input[] = $this->userInput;
                }

            } elseif ($this->userInput !== null) {
                $this->phases[$loop]->input[] = $this->userInput;
            }
    ...
    {
        $phases = $this->configurePhases($options);
    ...
    {
        while (true) {
            if ($this->phases[$phase]->halt) {
                throw new \Exception('phase '.$phase.' is halt');
            }
        ...
        $output = $this->phases[$phase]->out;
        $this->phases[$phase]->out = [];
